...
def test_valid_dict_delegated_unchanged_to_adapter(
    ...
    handler = create_lambda_handler(mock_adapter)

    response = handler(valid_payload, None)

    mock_adapter.handle.assert_called_once_with(valid_payload)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"claim_token": "token-123"},  # evaluation rejection / None result
    ],
)
